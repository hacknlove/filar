...
const { initializeCustomComponents, customComponents } = require('./customComponents');
...
describe('initializeCustomComponents', () => {
    ...
    it('should warn if component name does not match the convention', async () => {
        ...
        expect(console.warn).toHaveBeenCalledWith('File testComponent.html does not match the component naming convention');
    });
});

describe('customComponents', () => {
    it('should return a warning if the component does not exist', () => {
        expect(customComponents['wrong-component']).toBe('<!-- wrong-component does not exists -->');
    });
});